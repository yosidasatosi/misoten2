...
public class ParticleLineScaler : EditorWindow {
	...
	[MenuItem("Window/Particle Line Scaler")]

	public static void ShowWindow() {
		EditorWindow win = EditorWindow.GetWindow(typeof(ParticleLineScaler));
#if UNITY_5_1 || UNITY_5_2 || UNITY_5_3 || UNITY_5_4 || UNITY_5_5 || UNITY_5_6 || UNITY_5_7 || UNITY_5_8 || UNITY_5_9 || UNITY_2017
		win.titleContent = new GUIContent("ParticleLineScaler");
#else
			win.title = "ParticleLineScaler";	
#endif
		win.minSize = new Vector2(200.0f, 140.0f);
		win.maxSize = new Vector2(200.0f, 140.0f);
	}
	...
	public void OnGUI() {
		...
		GUILayout.Space(20);
		Color32 colorBlueLight = new Color32((byte)175, (byte)200, (byte)255, (byte)255);
		...
		GUIStyle styleToggle = new GUIStyle();
		styleToggle = new GUIStyle(GUI.skin.toggle);
		styleToggle.fontSize = 9;
		GUIStyle titleStyle = new GUIStyle(GUI.skin.label);
		titleStyle.fixedWidth = 200.0f;
		GUIStyle extraStyle = new GUIStyle();
		extraStyle.fontSize = 9;
		extraStyle.normal.textColor = colorBlueLight;
		EditorGUILayout.BeginHorizontal();
		GUILayout.FlexibleSpace();
		GUILayout.Label("PARTICLE LINES EDITION", extraStyle);
		GUILayout.FlexibleSpace();
		...
		if (GUILayout.Button("Scale", styleBigButton)) {
			foreach (GameObject gameObj in Selection.gameObjects) {
				...
				foreach (ParticleSystem ps in pss) {
					...
					ps.Play();
					LineScale(ps.transform.GetComponentInChildren<ParticleLines>());
	...
	public void LineScale(ParticleLines line){
		if (line == null) return;
		line.startScaleMultiplier *= scaleMultiplier;
		line.endScaleMultiplier *= scaleMultiplier;
	}
